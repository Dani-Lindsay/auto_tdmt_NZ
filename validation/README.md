...
> **STATUS (2026-09-07): the numbers on this page describe station
> selection v3.** The pipeline now runs **selection v5** — the
> published-rules version (Clinton loop, bounded depth grid, INGV
> grades; see [`docs/METHOD.md`](../docs/METHOD.md) §3, §5, §6 and
> [`docs/lit_review/`](../docs/lit_review/)). A v5 resweep of 2026 is
> the next step; until this page is regenerated from it, treat every
> number below as describing the previous pipeline. The `Selection`
> column in the catalogue says which version made each row.
...
|---|---|---|
| **NZ regional CMTs** (Ristau 2008 method; [GeoNet/data moment-tensor](https://github.com/GeoNet/data/tree/main/moment-tensor)) | Published regional MT solutions using the same method family and the same published velocity models as this pipeline | The *automation*: with method and models shared, disagreement isolates the effect of the automated station selection, band choice and depth search. The largest sample (n = 39 matches) and the primary benchmark. |
...
**1.0 km** for A/B grades. When the quality gates pass, the centroid
depths agree with the reference at the kilometre level; the reference
depths are not an input to the search.
...
**Context for 18°**: for the nine events where both Ristau and USGS
publish solutions, the two reference catalogues differ from *each
other* by a median of **18°** (individual events range 7–81°) — the
dashed line on the figure. The A/B tier therefore agrees with the
references at about the level the references agree with each other.
