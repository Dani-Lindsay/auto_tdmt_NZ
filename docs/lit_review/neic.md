# USGS NEIC — operational moment-tensor practice and quality control

Scope note: NEIC's **regional** moment tensors (Mwr, 0–10°) follow
Herrmann, Benz & Ammon (2011), which is the practice directly comparable
to this pipeline. The W-phase and SynDepth material below is
**teleseismic** and is included for two transferable design elements
only — the two-stage screening architecture and the magnitude-scaled
time-shift cap — not as a method to follow.

Findings gathered September 2026. Full-text obtained for Hayes, Rivera &
...
Implication: this pipeline's 8 s bound sits inside published operational practice.
...
nothing.
Implication: the peak-to-noise metric used by earlier versions of this pipeline was the same kind of measurement (peak
...
local minimums**".
Implication: a precedent for using the hypocentre to bound the search, not the
...
Implication: flagging a grid-edge depth is standard practice. Their
response (widen the grid) differs from ours (prefer the best interior
local maximum); worth considering.

**Mislocation from time shifts (c).** The SLU/USGS
...
Implication: this pipeline currently only clips the shift. Fitting the pattern would tell us whether a
...
Implication: this pipeline's 10-50 s band is 0.02-0.10 Hz and its processing floor is
preliminary M 3.7; both match the published North American regional
...
Implication: this pipeline uses 2.5 km/s plus a magnitude-dependent tail, envelope-extended.
...
Implication: no NEIC threshold can be cited; a regional pipeline has to derive and defend its own.
